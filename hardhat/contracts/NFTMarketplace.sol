...
contract NFTMarketplace {
    ...
    function createListing(address nftAddress, uint256 tokenId, uint256 price) external isNotListed(nftAddress, tokenId) isNFTOwner(nftAddress, tokenId) {
        ...
        // Want to Check if the caller is owner of NFT and has approved the marketplace contract to transfer on their behalf
    ...
    function cancelListing(address nftAddress, uint256 tokenId) external isListed(nftAddress, tokenId) isNFTOwner(nftAddress, tokenId) {
        // Delete the Listing struct from the mapping...Freeing up storage saves gas! 
